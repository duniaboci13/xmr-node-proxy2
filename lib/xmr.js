...
function blobTypeXTM_T(blob_type_num) {
    return blob_type_num == 106;
}
...
function convertBlob(blobBuffer, blob_type_num){
    if (blobTypeDero(blob_type_num)) return Buffer.from(blobBuffer);
    else if (blobTypeXTM_T(blob_type_num)) return Buffer.from(blobBuffer)
...
function constructNewBlob(blockTemplateBuffer, NonceBuffer, blob_type_num, ring){
    if (blobTypeDero(blob_type_num) || blobTypeXTM_T(blob_type_num)) {
        ...
        return cnUtil.construct_block_blob(blockTemplateBuffer, NonceBuffer, blob_type_num, ring);
    }
}
...
function parse_blob_type(blob_type_str) {
    ...
    switch (blob_type_str) {
        ...
        case 'cryptonote_arq':  return 16; // ARQ
